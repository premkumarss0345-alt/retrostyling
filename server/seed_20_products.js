...
async function seed() {
  ...
  try {
    console.log('🌱 Adding 20 Premium Products to Database...');

    // 1. Get Category IDs
    const [categories] = await connection.execute('SELECT id, slug FROM categories');
    const catMap = Object.fromEntries(categories.map(c => [c.slug, c.id]));

    // Check if categories exist, if not seed them first (safety fallback)
    if (categories.length === 0) {
        console.log('⚠️ No categories found. Seeding categories first...');
    ...
    // 2. Clear old products to avoid duplicates during this task (optional)
    ...
    console.log('✅ Success! 20 products and their variants added.');
  } catch (err) {
